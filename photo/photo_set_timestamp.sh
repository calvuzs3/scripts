...
SetTimestampByExif() {
  ...
  do 
    ...
    # Elaboriamo
    if [ -n "$filedate" ]; then
      exiftool "-ModifyDate=$filedate" \
        "-QuickTime:TrackCreateDate=$readable_date" \
        "-QuickTime:TrackModifyDate=$filedate" \
        "-QuickTime:MediaCreateDate=$filedate" \
        "-QuickTime:MediaModifyDate=$filedate" \
        "-overwrite-original" \
        "$file"
