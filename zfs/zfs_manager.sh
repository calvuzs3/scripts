...
create_refind_boot_environment() {
    ...
    local be_dataset="$POOL/ROOT/BE-$be_name"
    local source_snapshot="$POOL/ROOT/calvuz@$snapshot_name"
...
create_recovery_environment() {
    ...
    if ! zfs snapshot "$POOL/ROOT/calvuz@$snapshot_name"; then
...
list_refind_boot_environments() {
    ...
    if dataset_exists "$POOL/ROOT/calvuz"; then
        local used=$(zfs get -H -o value used "$POOL/ROOT/calvuz")
        local creation=$(zfs get -H -o value creation "$POOL/ROOT/calvuz")
    ...
    while read -r dataset; do
        if [[ "$dataset" =~ ^${POOL}/ROOT/BE- ]]; then
            local be_name="${dataset#$POOL/ROOT/BE-}"
    ...
    done < <(zfs list -H -o name -s creation | grep "^$POOL/ROOT/")
...
activate_refind_boot_environment() {
    ...
    if dataset_exists "$POOL/ROOT/calvuz"; then
        if ! zfs rename "$POOL/ROOT/calvuz" "$POOL/ROOT/$backup_name"; then
    ...
    if ! zfs rename "$be_dataset" "$POOL/ROOT/calvuz"; then
        log "✗ Errore attivazione Boot Environment"
        # Tentativo di rollback
        zfs rename "$POOL/ROOT/$backup_name" "$POOL/ROOT/calvuz" 2>/dev/null
        return 1
    fi
    
    # 5. Configura proprietà del nuovo sistema principale
    zfs set canmount=on "$POOL/ROOT/calvuz"
    zfs set mountpoint=/ "$POOL/ROOT/calvuz"
    
    # 6. Pulisce e ricostruisce configurazione rEFInd
    clean_be_entries
    
    # 7. Aggiunge entry per il sistema di backup
    add_be_entry_to_refind "$backup_name" "$POOL/ROOT/$backup_name" "Sistema Precedente (Backup)" true
...
destroy_refind_boot_environment() {
    ...
    while read -r dataset; do
        if [[ "$dataset" =~ ^${POOL}/ROOT/BE- ]]; then
            local remaining_be="${dataset#$POOL/ROOT/BE-}"
        ...
        elif [[ "$dataset" =~ ^${POOL}/ROOT/backup- ]]; then
            local backup_name="${dataset#$POOL/ROOT/}"
            add_be_entry_to_refind "$backup_name" "$dataset" "Sistema di Backup" true
        fi
    done < <(zfs list -H -o name | grep "^$POOL/ROOT/" | grep -v "^$POOL/ROOT/calvuz$")
...
cleanup_temporary_environments() {
    ...
    while read -r dataset; do
        if [[ "$dataset" =~ ^${POOL}/ROOT/(BE-rollback-|BE-recovery-|backup-) ]]; then
            local be_name="${dataset#$POOL/ROOT/BE-}"
            [[ "$dataset" =~ ^${POOL}/ROOT/backup- ]] && be_name="${dataset#$POOL/ROOT/}"
    ...
    done < <(zfs list -H -o name | grep "^$POOL/ROOT/")
...
promote_boot_environment() {
    ...
    local be_dataset="${ZFS_ROOT}/${be_name}"
    ...
    if [[ "$current_dataset" == "${ZFS_ROOT}/$(basename ${ZFS_ROOT})" ]]; then
    ...
    # Rinomina il Boot Environment promosso al nome standard (se diverso)
    local standard_name=$(basename "$ZFS_ROOT")
    if [[ "$be_name" != "$standard_name" ]]; then
        echo "🔄 Rinomina Boot Environment al nome standard..."
        if ! zfs rename "$be_dataset" "${ZFS_ROOT}/${standard_name}"; then
            echo "❌ Errore nella rinomina del Boot Environment"
            return 1
        fi
        be_dataset="${ZFS_ROOT}/${standard_name}"
...
check_promotion_status() {
    local expected_dataset="${ZFS_ROOT}/$(basename ${ZFS_ROOT})"
...
show_refind_menu() {
    ...
    echo " I) Setup/Verifica include Boot Environment"  # NUOVA OPZIONE
    echo " V) Visualizza file configurazione BE"        # NUOVA OPZIONE
    echo
    ...
    while true; do
        ...
        case $choice in
            ...
            3)
                ...
                list_refind_boot_environments | grep "✓.*BE-" | head -10
                echo
